...
void main() {
  ...
  p += vec2(
    sin(uTime * (0.18 + aSeed * 0.14) + phase + p.y * 4.0),
    cos(uTime * (0.15 + aSeed * 0.12) + phase + p.x * 4.0)
  ) * 0.004;

  vec2 mouseDelta = p - uMouse;
  float mouseDistance = length(mouseDelta);
  float push = smoothstep(0.32, 0.0, mouseDistance) * uMouseActive;
  ...
  gl_PointSize = aSize * uPixelRatio * (1.0 + push * 0.75);
  vSeed = aSeed;
  vAlpha = 0.74 + 0.26 * sin(phase + uTime * (0.34 + aSeed * 0.28));
}`;
  ...
  useEffect(() => {
    ...
    image.onload = () => {
      ...
      try {
        ...
        sourceContext.clearRect(0, 0, 420, 420);
        sourceContext.drawImage(image, 0, 0, 420, 420);
        const pixels = sourceContext.getImageData(0, 0, 420, 420).data;
        const candidates: Array<[number, number]> = [];
        for (let y = 0; y < 420; y += 2) {
          for (let x = 0; x < 420; x += 2) {
            if (pixels[(y * 420 + x) * 4 + 3] > 72) candidates.push([x, y]);
          }
        }
        ...
        for (let i = 0; i < count; i++) {
          ...
          const s = Math.random();
          seed[i] = s;
          size[i] = s > 0.965 ? 20 + Math.random() * 10.4 : s > 0.76 ? 10 + Math.random() * 6.4 : 6 + Math.random() * 4.8;
        }
        ...
        const render = (milliseconds: number) => {
          ...
          gl.uniform1f(uniforms.time, reducedMotion ? 0 : milliseconds / 1000 - startedAt);
